...
if (config.logWebhook.length > 25) {
  ...
  log.setUsername("STOLEN | bocuthohang - Poketwo Catcher");
  log.setAvatar(
    "https://i.pinimg.com/736x/5a/cd/73/5acd731a12d44a4202166270a48381f1.jpg"
  );
}
    ...
    client.on("ready", async () => {
      ...
      async function interval(intervals) {
        if (!isOnBreak && !captcha) {
          ...
          if (randomInteger(15000, 60000) === 400 && config.sleeping) {
      ...
      intervals = Math.floor(Math.random() * (60000 - 15000 + 1)) + 15000;
